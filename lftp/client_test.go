...
func TestParseDirList(t *testing.T) {
	...
	expected := []File{
		File{Created: time.Date(2014, 6, 25, 14, 15, 16, 0, time.FixedZone("CEST", 7200)), Path: "dir1"},
		File{Created: time.Date(2015, 2, 2, 23, 1, 15, 0, time.FixedZone("CET", 3600)), Path: "dir2"},
		File{Created: time.Date(2015, 3, 15, 8, 28, 30, 0, time.FixedZone("CET", 3600)), Path: "dir3"},
	}
	...
	for i, e := range expected {
		a := actual[i]
		if !e.Created.Equal(a.Created) || e.Path != a.Path {
